...
import com.dither.handler.GroupUserHandler;
...
public class ProxyVerticle extends AbstractVerticle {
  ...
  public void start(Future<Void> future) {
    ...
    JsonObject proxyConfig = injector
        .getInstance(Key.get(JsonObject.class, Names.named("config.proxy")));
    ...
    router.route().handler(injector.getInstance(GroupUserHandler.class));
    router.route().handler(injector.getInstance(ProxyServiceHandler.class));

    vertx.createHttpServer().requestHandler(router::accept)
        .listen(proxyConfig.getInteger("port"), proxyConfig.getString("host"),
            result -> {
              if (result.succeeded()) {
                future.complete();
              } else {
